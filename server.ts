...
// MED-1: validate a request body against a strict zod schema. On failure responds 400
...
async function startServer() {
  ...
  // MED-3: rate limiting. In-memory store (default) — counters reset on restart and are
  ...
  // SSE stream — segmented per tenant/table (ALTO-3). Soft auth: a valid session cookie
  ...
  app.post('/api/auth/login', loginLimiter, (req, res, next) => {
    try {
      ...
      // MED-2: run verifyPassword in BOTH branches (dummy hash when the user is unknown)
  ...
  // ALTO-1: scoped diner order lookup. Returns only orders that belong to this tenant
  // AND table AND whose ids the caller already holds (from its own session storage).
  ...
  // recomputed server-side from the catalog (ALTO-2). Whole order fails atomically.
  app.post('/api/establishments/:id/orders', (req, res, next) => {
    try {
      const body = parseBody(createOrderSchema, req, res);
      if (!body) return;

      const result: CreateOrderResult = store.createOrder({
  ...
  app.patch('/api/orders/:id/status', requireAuth, (req, res, next) => {
    try {
  ...
  app.post('/api/menu-items', requireAuth, requireRole('admin'), (req, res, next) => {
    try {
      ...
      const saved = store.saveMenuItem({
        id: body.id,
  ...
  app.delete('/api/menu-items/:id', requireAuth, requireRole('admin'), (req, res, next) => {
    try {
      const estId = req.user!.establishmentId;
  ...
  app.post('/api/categories', requireAuth, requireRole('admin'), (req, res, next) => {
    try {
  ...
  app.delete('/api/categories/:id', requireAuth, requireRole('admin'), (req, res, next) => {
    try {
      const estId = req.user!.establishmentId;
  ...
  app.post('/api/tables', requireAuth, requireRole('admin'), (req, res, next) => {
    try {
      const body = parseBody(saveTableSchema, req, res);
      if (!body) return;

      const estId = req.user!.establishmentId;
      const saved = store.saveTable({
        id: body.id,
        establishmentId: estId,
        name: body.name,
        active: body.active,
  ...
  app.delete('/api/tables/:id', requireAuth, requireRole('admin'), (req, res, next) => {
    try {
      const estId = req.user!.establishmentId;
  ...
  // MED-4: central error handler. Endpoints delegate here via next(e); we log the real
  // detail server-side and return a generic message so internals never leak to clients.
