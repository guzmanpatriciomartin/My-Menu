...
// Minutes to add to UTC to get venue-local time (negative for Argentina).
...
// Business day (YYYY-MM-DD) a given instant belongs to, in venue-local time.
export function venueDay(date: Date = new Date()): string {
  // Shifting back by the start hour makes early-morning sales fall into the previous
  // business day when BUSINESS_DAY_START_HOUR > 0.
  const shifted = new Date(date.getTime() - BUSINESS_DAY_START_HOUR * MS_PER_HOUR);
  const p = localParts(shifted);
  return `${p.year}-${pad(p.month)}-${pad(p.day)}`;
}

// Hour bucket (0-23) of an instant, in venue-local time.
...
// UTC bounds [from, to) of a business day given as YYYY-MM-DD in venue-local time.
export function dayBounds(day: string): { from: string; to: string } {
  const [year, month, dayOfMonth] = day.split('-').map(Number);

  // Start from the naive UTC instant, then correct by the real offset at that moment.
  // One refinement pass is enough: the offset only shifts on a DST boundary, and it
  // converges because the correction is at most a couple of hours.
...
// Shift a YYYY-MM-DD business day by N days (negative to go back).
...
// True when `day` is the business day currently in progress.
export function isToday(day: string): boolean {
  return day === venueDay();
}

// How far into the business day we are, in ms. Used to compare "same hour yesterday"
// instead of today-so-far against yesterday's full total (which always looks worse).
